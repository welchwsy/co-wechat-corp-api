 ...
 */
exports.createDepartment = async (name, opts) => {
  // https://qyapi.weixin.qq.com/cgi-bin/department/create?access_token=ACCESS_TOKEN
  var token = await this.ensure();
  
  var url = this.prefix + 'department/create';
  var data = {
    name: name
  };
...
exports.deleteDepartment = async (id) => {
  ...
  var token = await this.ensure();
  var url = this.prefix + 'department/delete';
  url += '&' + querystring.stringify({id: id});
  var opts = {dataType: 'json'};
  return await this.preRequest(url, opts);
};
